...
export function HomeSearch() {
  ...
  useEffect(() => {
    detectCountry().then(setOrigin).catch(() => setOrigin("US"));
  }, [detectCountry]);
  ...
  return (
    <div className="mx-auto w-full max-w-6xl">
      ...
      <p className="mx-auto mt-5 max-w-3xl text-center text-sm leading-relaxed text-muted-foreground">{t("search.guide")}</p>
      <div className="mt-5 flex items-center justify-center text-[11px] font-bold uppercase tracking-[0.13em] text-muted-foreground">
        <span className="inline-flex items-center gap-2"><CircleCheck className="h-3.5 w-3.5 text-accent" />{t("search.promise")}</span>
      </div>
    </div>
  );
}