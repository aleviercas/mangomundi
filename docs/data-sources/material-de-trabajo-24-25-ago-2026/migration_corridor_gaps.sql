...
-- investigacion-corredores-dinamicos-agosto-2026.md seccion 3 para el porque).
--
...
-- Western Union: tramo regular >=1000 GBP (el tramo <1000 muestra una tasa
-- distinta y se descarta por posible promocional, ver doc de research).
...
-- MoneyGram: tramo regular 5000-10000 GBP (tramo 100-1000 es promo "1st transfer", no cargado)
insert into fx_rates (from_currency, to_currency, rate, fee, provider_slug, sending_country, receiving_country, is_local_fx, public_spread_percent, min_amount, max_amount, data_source, data_collected_at, verified_status)
...
-- Ria: fee normal (no promo) escalona por monto, tasa plana 130.21. Tope real del corredor: 8000 GBP.
...
-- Xoom: 3 tramos reales (no promocionales, no requieren login)
insert into fx_rates (from_currency, to_currency, rate, fee, provider_slug, sending_country, receiving_country, is_local_fx, public_spread_percent, min_amount, max_amount, data_source, data_collected_at, verified_status) values
...
-- Remitly: tasa "everyday" (se descarta "welcome rate" promocional, tope 1000 GBP)
insert into fx_rates (from_currency, to_currency, rate, fee, provider_slug, sending_country, receiving_country, is_local_fx, public_spread_percent, min_amount, max_amount, data_source, data_collected_at, verified_status)
...
-- WorldRemit: 2 tramos, sin lenguaje promocional detectado
insert into fx_rates (from_currency, to_currency, rate, fee, provider_slug, sending_country, receiving_country, is_local_fx, public_spread_percent, min_amount, max_amount, data_source, data_collected_at, verified_status) values
...
-- ---------------------------------------------------------------------
-- Paysend: spread inusualmente ajustado (0.02%) -- se carga igual (es el
-- dato real observado, no promocional) pero queda marcado sin_confirmar
-- porque no se puede descartar un recargo oculto post-login no visible
-- en el cotizador publico. Recomendado re-verificar.
insert into fx_rates (from_currency, to_currency, rate, fee, provider_slug, sending_country, receiving_country, is_local_fx, public_spread_percent, min_amount, max_amount, data_source, data_collected_at, verified_status)
...
-- Spread negativo = tasa mejor que el mid-market oficial (aprovechan la
-- brecha oficial/paralelo del NGN). No promocional -- calculadoras publicas
-- de marketing sin login ni condicion de "primera vez".
-- ---------------------------------------------------------------------
insert into fx_rates (from_currency, to_currency, rate, fee, provider_slug, sending_country, receiving_country, is_local_fx, public_spread_percent, min_amount, max_amount, data_source, data_collected_at, verified_status) values
...
-- (ver docs/data-sources/... auditoria-cobertura-corredores). Western Union,
-- Remitly, MoneyGram y Ria ya tenian corredores hacia Argentina desde
-- España/EEUU pero NINGUNO tenia Reino Unido->Argentina cargado, pese a
-- operarlo activamente hoy. Mid-market referencia (xe.com, 25 ago 2026,
-- 08:49 UTC): 1 GBP = 2058.8092 ARS.
-- ---------------------------------------------------------------------
insert into fx_rates (from_currency, to_currency, rate, fee, provider_slug, sending_country, receiving_country, is_local_fx, public_spread_percent, min_amount, max_amount, data_source, data_collected_at, verified_status) values
...
-- ---------------------------------------------------------------------
-- Global66: se confirma por fuente primaria que el corredor Europa(EUR)->
-- Argentina(ARS) es real (remesa a tercero, no solo wallet propia), algo
-- que las notas anteriores (ago 2025) no reflejaban. No se pudo obtener
-- cifra de fee/spread en vivo (cotizador 100% dinamico, contencion de
-- navegador el 25-ago). Se documenta el hallazgo cualitativo, se mantiene
-- active=false y fee/spread en 0 hasta poder muestrear en vivo sin contencion.
-- ---------------------------------------------------------------------
update providers
...
-- paginas de producto independientes (mismo texto en ambas, confirma que
-- es tarifa general y no especifica de un corredor). Spread/margen
-- cambiario NO publicado por Prex en ningun lado -- se deja como
-- estimacion provisoria conservadora (1.0%), explicitamente marcada como
-- tal en las notas, hasta poder verificarlo con una cotizacion en vivo.
-- No se cargan filas en fx_rates porque no hay una "rate" (tipo de
-- cambio ofrecido) verificable con fuente primaria -- cargar un numero
-- inventado ahi violaria la regla de nunca inventar datos.
-- ---------------------------------------------------------------------
