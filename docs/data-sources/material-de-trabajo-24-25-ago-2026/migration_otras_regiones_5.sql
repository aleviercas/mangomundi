...
-- (GB->IN, GB->PK, GB->PH), TapTap Send (FR->SN, GB->UG). Elegidos entre las
-- matrices de expansion documentadas en
-- docs/data-sources/2026-08-25b-auditoria-cobertura-otras-regiones.md.
--
-- Regla: solo precio regular. LemFi GB->PK se carga como sin_confirmar: el
-- fee pasa de 0.99 GBP (100 GBP) a 0.00 GBP (500 GBP) sin ninguna etiqueta
-- que aclare si es un tramo de fee estandar o una promo "envia mas, paga
-- menos" sin badge visible. El resto (7 corredores) confirmado_activo: sin
-- banners promocionales, y en el caso de Sendwave GB->IN se verifico
-- explicitamente que el rotulo "Intro Rate Discount" (que SI aparece en su
-- corredor GB->PH) esta ausente para India.
--
