...
| **RemitSCOPE** (Banco Mundial + CGAP/GIZ) | Agrega RPW + datos de bancos centrales + PRIME Africa; fuerte en LatAm y África | Regional (África y LatAm ya, Asia "próximamente"); explícitamente admite huecos de cobertura | Gratis, plataforma web (no confirmé API) |
...
Pediste conectar "las que ya tenemos conectadas por API" — aclaro qué son exactamente, porque no cubren lo que hace falta: Frankfurter, ExchangeRate-API, Fixer.io, exchangeratesapi.io y Open Exchange Rates (las 5 en `src/lib/fx.functions.ts`) **solo dan la tasa de referencia del mercado interbancario** (el "mid-market rate" contra el que se mide todo lo demás). Ninguna de esas cinco sabe qué comisión o margen cobra Western Union, Wise, MoneyGram, etc. — eso es exactamente el dato que falta y que hoy se consigue solo a mano (research directo) o trimestralmente (World Bank). La Wise Comparison API es la única fuente de la lista de la sección 4 que sí devuelve precios de competidores, no solo la tasa de mercado — por eso es el hallazgo más importante.

### 4.2 Cambio de moneda dentro del mismo país (FX local) — hoy en cero, hace falta empezar de cero

Encontré algo relevante: el esquema de `fx_rates` **ya tiene una columna `is_local_fx boolean`** — alguien ya había anticipado este caso de uso (convertir moneda dentro de un mismo país, ej. USD↔ARS en una casa de cambio en Argentina, sin que haya ningún envío internacional de por medio). Pero hoy tiene **cero filas** — es un campo del esquema sin datos, un área completamente sin construir todavía.

Esto es un dominio de datos distinto al de remesas — cada país tiene un mercado cambiario local con su propia estructura (bancos, casas de cambio, mercado paralelo, tipos de cambio múltiples), así que no hay una fuente única global como el World Bank para esto. Para Argentina específicamente (el caso que motivó esta conversación) encontré fuentes abiertas y gratuitas que ya resuelven justo este problema:
...
Esto resuelve Argentina (y de yapa varios países de LatAm con regímenes cambiarios similares) casi gratis y en vivo. Para el resto del mundo, esto va a necesitar una investigación país por país — la mayoría de los países no tienen un "dólar blue", tienen un mercado bancario/casas de cambio más simple (ej. UK: bureaux de change como ICE, Travelex), así que la fuente y la estructura de datos van a variar mucho más que en remesas cross-border. Lo marco como su propio frente de trabajo, no como algo que se resuelve con la misma receta que World Bank/Wise.
...
Vale la pena escribirle a `comparison@wise.com` para entender si mangomundi califica como partner. Si da acceso, resolvería de raíz el problema de "dato desactualizado/faltante" para transferencias banco-a-banco (que son la mayoría del volumen), dejando la investigación manual solo para lo que esa API no cubre: cash pickup, mobile money, especialistas regionales (hawala, M-Pesa, etc.) — que es donde ya venís invirtiendo research manual de todos modos.

### 5.6 Motor de búsqueda/ranking — ya está bien encaminado

Repasé `scoring.functions.ts` a fondo: el sistema multi-criterio (10 perfiles de score, normalización min-max, tie-breaks estadísticamente honestos, auditoría de Pareto-dominancia, detección de tendencia de trust score) es un diseño **sofisticado y ya de nivel "mejor del mundo"** — de hecho está mejor pensado que lo que hacen la mayoría de los comparadores públicos que revisé. No encontré nada urgente para optimizar ahí. El cuello de botella real de calidad no es el motor de ranking, es la cobertura de datos que ese motor recibe como input — un ranking perfecto sobre datos incompletos sigue dando resultados incompletos, que es exactamente el síntoma que reportaste.

---

## 6. Plan de acción priorizado

1. **Hecho en esta sesión:** research y documentación consolidados en este archivo, corrección de la cifra incorrecta de Western Union (ver recuadro arriba), y publicación de todo esto en `docs/` del repo de GitHub (ver sección 8) para que cualquier sesión de Claude con acceso al repo lo tenga disponible sin depender de este chat.
2. **Inmediato, bajo riesgo:** cargar a mano el corredor UK→Argentina (y los corredores UK→LatAm más pedidos) para los proveedores corridor-specific relevantes, con el fee real por rango de monto (no un número fijo — ver la corrección de arriba) y SIEMPRE citando la fuente primaria (el sitio del proveedor), nunca un agregador sin verificar.
...
7. **Continuo:** seguir el patrón ya establecido en `docs/multi-criteria-ranking/` — cada dato nuevo con fuente y fecha, nunca inventado, nunca corrido contra producción sin tu aprobación explícita.

---

## 7. Qué necesito confirmar antes de tocar producción

Todo lo de arriba (y lo cargado a GitHub) es diagnóstico, research y documentación — no toqué la base de Supabase todavía, siguiendo el mismo criterio que ya venías usando con el agente anterior ("no se corre nada contra producción hasta que Alejandro lo apruebe explícitamente"). Antes de escribir cualquier dato nuevo en `fx_rates`/`providers` o de activar la integración con dolarapi/bluelytics en el código, necesito que confirmes:

- ¿Arranco por el fix rápido de UK→Argentina y corredores UK→LatAm similares (con fuente primaria, sin la cifra incorrecta de antes)?
- ~~¿Querés que le escriba a Wise...?~~ **Hecho** — ya le escribiste directo a `comparison@wise.com`. Queda pendiente su respuesta; cuando llegue, la anoto acá (sección 9) y seguimos según lo que digan.
- Para el FX local (dólar blue/MEP/CCL): ¿lo tratamos como una feature nueva del comparador (una pestaña separada de "conversión local" vs. "envío internacional") o como un dato más dentro del mismo flujo? Esto cambia bastante el diseño de la UI, no solo la carga de datos.
- ¿Doy luz verde para el cambio de código de "exclusión dura" → "degradación con transparencia", o preferís revisarlo en una rama primero?

---

## 8. Dónde vive este documento

Publicado en `docs/data-sources/2026-08-diagnostico-arquitectura-proveedores-corredores.md` en el repo `aleviercas/mangomundi` (mismo patrón de carpeta que `docs/multi-criteria-ranking/`), para que cualquier sesión de Claude con acceso al repo — no solo esta conversación — pueda leerlo, seguir el research desde donde quedó, y no repetir trabajo ya hecho. También queda espejado en el proyecto de Claude "Mangomundi". A medida que se sume más research (FX local país por país, respuesta de Wise, etc.) se actualiza este mismo archivo en vez de crear versiones sueltas — mismo criterio que ya usaban los docs de `multi-criteria-ranking`.

Además, `ale.md` y `MIGRATION.md`, que vivían sueltos en la raíz del repo, se movieron a `docs/` para que exista una sola carpeta donde buscar toda la documentación — ver `docs/README.md` para el índice completo.

---

## 9. Email a Wise (Comparison API) — ✅ ya enviado

Alejandro ya le escribió directo a `comparison@wise.com` (24 ago 2026). Queda en espera de respuesta — cuando llegue, avisar para actualizar la sección 5.5/6 del plan según lo que Wise conteste (acceso sí/no, condiciones, cobertura real). El borrador que se había preparado queda abajo como referencia de qué se preguntó:
